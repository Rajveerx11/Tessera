...
export function AiActionPanel() {
  ...
  const actions = [
    { id: 'plan', label: 'Test Plan', icon: <TerminalSquare className="w-4 h-4" /> },
    { id: 'cases', label: 'Test Cases', icon: <CheckSquare className="w-4 h-4" /> },
    { id: 'unit', label: 'Unit Tests', icon: <Code2 className="w-4 h-4" /> },
    { id: 'integration', label: 'Integration', icon: <Layers className="w-4 h-4" /> },
    { id: 'review', label: 'Code Review', icon: <SearchCode className="w-4 h-4" /> },
  ...
  return (
    <div className="flex flex-col h-full bg-background overflow-hidden border-l border-border/50">
      ...
      <div className="p-4 flex flex-col gap-6 shrink-0 border-b border-border/50">
        <div className="space-y-3">
          <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Scope</label>
          <div className="flex p-0.5 bg-muted rounded-md border border-border/50">
            <button 
              onClick={() => setScope('current')}
              className={cn(
                "flex-1 py-1 text-[10px] font-bold uppercase tracking-tight rounded transition-all",
        ...
        <div className="space-y-3">
          <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Actions</label>
          <div className="grid grid-cols-2 gap-1.5">
            {actions.map((action) => (
              <button
                ...
                className={cn(
                  "flex items-center gap-2.5 p-2 rounded border border-border/50 transition-all text-left",
                  isGenerating ? "opacity-50 cursor-not-allowed" : "hover:bg-muted/50 hover:border-border"
                )}
              >
        ...
        {isGenerating && (
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between text-[9px] text-muted-foreground font-mono">
              <span className="truncate flex-1 uppercase">Generating {generationState.currentFile}...</span>
              <span>{generationState.status === 'streaming' ? generationState.progress : 0}%</span>
            </div>
          </div>
        )}
      </div>
